...
interface ComponentsDao {
    ...
    @Query("select c.* from components c join libraries l on l.id = c.libraryId and l.name = :libraryName,component_markings cm on cm.componentId = c.id, marking_types mt on mt.id = cm.markingId and mt.literalValue = 'pin' order by c.name asc")
    fun getAllPinnedComponentsByLibraryName(libraryName: String): List<Component>

    @Query("select c.* from components c join libraries l on l.id = c.libraryId and l.name = :libraryName where not exists (select * from component_markings where componentId = c.id) order by c.name asc")
